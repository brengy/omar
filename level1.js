...
function generatePlatforms() {
  if (platforms.length === 0 || platforms[platforms.length - 1].x - camera.x < canvas.width - 200) {
    ...
    const minGap = 180; // ÊÞáíá ÇáÝÌæÉ ÇáÏäíÇ áÊÓåíá ÇáÞÝÒ
    const maxGap = 220; // ÊÞáíá ÇáÝÌæÉ ÇáÞÕæì áÊÓåíá ÇáÞÝÒ
    ...
    yPos = Math.max(minHeight, Math.min(yPos, maxHeight)); // ÖãÇä Ãä Êßæä ÇáÃáæÇÍ Öãä ÇáÍÏæÏ ÇáãÞÈæáÉ

    const platform = new Platform(xPos, yPos, platformWidth, platformHeight, getRandomColor());
    platforms.push(platform);

    // ÅÚÏÇÏ ÇááÇÚÈ áíÈÏÃ ÝæÞ ÇááæÍ ÇáÃæá
...
function generateCoins() {
  platforms.forEach(platform => {
    if (!platform.coinsGenerated) {
      const numberOfCoins = Math.floor(Math.random() * 3) + 1; // ÊÞáíá ÚÏÏ ÇáÚãáÇÊ
...
function handlePlayerMovement() {
  if (isMobile) {
    // ÇáÍÑßÉ ÇáÊáÞÇÆíÉ ãä ÇáíÓÇÑ Åáì Çáíãíä
    player.x += player.speed;
  } else {
    // ÇáÍÑßÉ ÈÇÓÊÎÏÇã ÇáÃÒÑÇÑ Úáì ÇáÃÌåÒÉ ÛíÑ ÇáãÍãæáÉ
...
function handlePlayerVerticalMovement() {
  ...
  platforms.forEach(platform => {
    if (player.x < platform.x + platform.width &&
      ...
      player.y = platform.y - player.height;
      player.velocityY = 0;
      player.isJumping = false; // ÇáÓãÇÍ ÈÇáÞÝÒ ãÑÉ ÃÎÑì ÚäÏ áãÓ ÇáãäÕÉ
    }
  });
...
function update() {
  if (!gameOver) {
    ...
    render();
  if (player.score >= 50) {
      window.location.href = 'level2.html'; // ÇáÇäÊÞÇá Åáì ÇáãÓÊæì 2
      return; // ÅíÞÇÝ ÇáÊÍÏíË áãäÚ ÇáãÒíÏ ãä ÇáÚãáíÇÊ
    }
...
function drawUI() {
  // ÚÑÖ ÇáãÓÊØíá
  const boxWidth = 150;
  // ãæÞÚ ÇáãÓÊØíá ÇáÃÝÞí (ãäÊÕÝ ÇáßÇäÝÓ)
  const boxX = (canvas.width - boxWidth) / 2;

  // ÇáãæÞÚ ÇáÑÃÓí ááãÓÊØíá ÇáÃæá ÈÍíË íßæä ÊÍÊ ÈãÞÏÇÑ 20 ÈßÓá ãä ÇáßÇäÝÓ
  const boxY = canvas.height - 20 - 70; // 70 åæ ÇáÇÑÊÝÇÚ Çáßáí ááãÓÊØíáíä æÇáãÓÇÝÉ ÈíäåãÇ

  // ÑÓã ÇáãÓÊØíá ÇáÃæá (Score)
  ...
  // ÑÓã ÇáãÓÊØíá ÇáËÇäí (High Score) ÃÓÝá ÇáãÓÊØíá ÇáÃæá
