...
window.addEventListener('keydown', (event) => {
 ...
 if (keySequence.join('') === 'sss') {
    window.location.href = 'bonus2.html';
  }
...
function generateEnemies() {
  if (Math.random() < 0.02) { // «Õ „«· ŸÂÊ— «·√⁄œ«¡
    ...
    const enemy = new Enemy(canvas.width + camera.x, enemyY, 50, 50, 6); // «·⁄œÊ Ì Õ—ﬂ »”—⁄… 2
    enemies.push(enemy);
  }
}
...
function update() {
  if (!gameOver) {
    ...
    render();
  if (player.score >= 70) {
      window.location.href = 'bonus2.html'; // «·«‰ ﬁ«· ≈·Ï «·„” ÊÏ 2
      return; // ≈Ìﬁ«› «· ÕœÌÀ ·„‰⁄ «·„“Ìœ „‰ «·⁄„·Ì« 
    }
