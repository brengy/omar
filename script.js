...
const player = {
  x: 50,
  y: 0, // سيتم تعيين هذه القيمة لاحقًا لتكون فوق اللوح الأول
  width: 130,
  height: 150,
  velocityX: 0,
  velocityY: 0,
...
function generatePlatforms() {
  if (platforms.length === 0 || platforms[platforms.length - 1].x - camera.x < canvas.width - 200) {
    ...
    const minGap = 50; // تقليل الفجوة الدنيا لتسهيل القفز
    const maxGap = 150; // تقليل الفجوة القصوى لتسهيل القفز
    ...
    yPos = Math.max(minHeight, Math.min(yPos, maxHeight)); // ضمان أن تكون الألواح ضمن الحدود المقبولة

    const platform = new Platform(xPos, yPos, platformWidth, platformHeight, getRandomColor());
    platforms.push(platform);

    // إعداد اللاعب ليبدأ فوق اللوح الأول
...
function generateCoins() {
  platforms.forEach(platform => {
    if (!platform.coinsGenerated) {
      const numberOfCoins = Math.floor(Math.random() * 3) + 1; // تقليل عدد العملات
...
function handlePlayerVerticalMovement() {
  ...
  if ((keys['Space'] || keys['ArrowUp'] || keys['KeyW']) && onPlatform) player.velocityY = -player.jumpHeight;
}
...
function drawUI() {
  ...
  drawRoundedRect(canvas.width - 155, 5, 150, 35, 5, 'white', 'black', 3);
  ctx.fillText(`High Score: ${highScore}`, canvas.width - 145, 30);
}
