...
function loadAssets(callback) {
  ...
  enemyImage.src = 'https://i.imgur.com/FkQZluV.png';
  callback();
}
...
const player = {
  ...
  bullets: [],
  shoot() {
    // ÅÖÇÝÉ ãÞÐæÝ ÌÏíÏ ÚäÏ ÅØáÇÞ ÇáäÇÑ
    this.bullets.push({
      x: this.x + this.width,
...
// ÅÖÇÝÉ ÍÏË ÅØáÇÞ ÇáäÇÑ ÚäÏ ÇáäÞÑ Úáì ÇáÒÑ Ýí ÇáÃÌåÒÉ ÇáãÍãæáÉ
if (isMobile) {
  const shootButton = document.getElementById('shootButton');
  shootButton.addEventListener('click', () => {
    player.shoot();  canvas.focus();// ÇÓÊÏÚÇÁ ÏÇáÉ ÅØáÇÞ ÇáäÇÑ ÚäÏ ÇáäÞÑ
  });
}
...
function generateEnemies() {
  if (Math.random() < 0.01) { // ÇÍÊãÇá ÙåæÑ ÇáÃÚÏÇÁ
...
function handlePlayerVerticalMovement() {
  ...
  // ÇáÓãÇÍ ÈÇáÞÝÒ ÝÞØ ÅÐÇ áã íßä ÇááÇÚÈ íØáÞ ÇáäÇÑ
...
function update() {
  if (!gameOver) {
    ...
    render();
  if (player.score >= 70) {
      window.location.href = 'level3.html';    return; 
    }
...
function detectCollisions() {
  ...
  // ÇáÊÍÞÞ ãä ÇáÇÕØÏÇã Èíä ÇáãÞÐæÝÇÊ æÇáÃÚÏÇÁ
  ...
  // ÇáÊÍÞÞ ãä ÇáÇÕØÏÇã Èíä ÇáÔÎÕíÉ æÇáÃÚÏÇÁ áÅäåÇÁ ÇááÚÈÉ
  enemies.forEach((enemy, index) => {
    if (player.x < enemy.x + enemy.width &&
        player.x + player.width > enemy.x &&
        player.y < enemy.y + enemy.height &&
        player.y + player.height > enemy.y) {
      gameOver = true; // ÅäåÇÁ ÇááÚÈÉ ÚäÏ ÇáÇÕØÏÇã
    }
  ...
  // ÅÒÇáÉ ÇáãÞÐæÝÇÊ ÇáÊí ÊÎÑÌ ãä ÇáÔÇÔÉ
...
window.addEventListener('keydown', (event) => {
  ...
  // ÅØáÇÞ ÇáäÇÑ ÚäÏ ÇáÖÛØ Úáì ãÝÊÇÍ ÇáãÓÇÝÉ
